...
ENV_VARS="NODE_ENV=production,SESSION_DURATION_DAYS=${SESSION_DURATION_DAYS},BODY_SIZE_LIMIT=${BODY_SIZE_LIMIT},TURSO_DATABASE_URL=${TURSO_DATABASE_URL},TURSO_AUTH_TOKEN=${TURSO_AUTH_TOKEN}"
...
# Use ORIGIN from env file if set, otherwise fall back to Cloud Run URL
FINAL_ORIGIN="${ORIGIN:-${SERVICE_URL}}"
echo "Setting ORIGIN=${FINAL_ORIGIN}..."
FINAL_ENV="ORIGIN=${FINAL_ORIGIN},SESSION_DURATION_DAYS=${SESSION_DURATION_DAYS},BODY_SIZE_LIMIT=${BODY_SIZE_LIMIT},TURSO_DATABASE_URL=${TURSO_DATABASE_URL},TURSO_AUTH_TOKEN=${TURSO_AUTH_TOKEN}"
if [[ -n "${GOOGLE_CLIENT_ID}" ]]; then

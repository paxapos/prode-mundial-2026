<script lang="ts">
	...
	/* ─── Tournament members ─── */
	const memberIds = $derived(new Set(data.tournamentMembers.map((u) => u.id)));
	const nonMembers = $derived(data.users.filter((u) => !memberIds.has(u.id)));
</script>
				...
				{#if nonMembers.length > 0}
					<form method="POST" action="?/addToTournament" use:enhance class="mb-4 flex flex-wrap items-end gap-3 rounded-lg border border-dashed border-emerald-300 bg-emerald-50/50 p-3">
						...
						<div class="flex-1">
							...
							<select name="userId" class="w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm">
								{#each nonMembers as user}
									<option value={user.id}>{user.nickname} ({user.email})</option>
								{/each}
							</select>
						</div>
						<button type="submit" class="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-bold text-white hover:bg-emerald-700">
							➕ Agregar
						</button>
					</form>
				{:else}

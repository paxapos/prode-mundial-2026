				...
				<table class="w-full text-sm">
					...
					<tbody class="divide-y divide-slate-50">
						{#each data.users as user}
							...
							{:else}
								<tr class="hover:bg-slate-50/50"><td class="py-2.5 pl-4"><div class="flex items-center gap-2"><span class="inline-flex h-7 w-7 items-center justify-center rounded-full bg-slate-200 text-[10px] font-bold text-slate-600">{user.nickname.charAt(0).toUpperCase()}</span><span class="font-semibold text-slate-800">{user.nickname}</span></div></td><td class="py-2.5 text-xs text-slate-500">{user.email}</td><td class="py-2.5 text-center"><span class="inline-flex rounded-full px-2 py-0.5 text-[10px] font-bold uppercase {user.role === 'admin' ? 'bg-red-100 text-red-700' : 'bg-sky-100 text-sky-700'}">{user.role === 'admin' ? '👑' : '🎮'} {user.role}</span></td><td class="py-2.5 text-center"><button type="button" onclick={() => toggleAudit(user.id)} class="rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-700 hover:bg-slate-200">{summary?.count ?? 0} cambios</button><p class="mt-1 text-[10px] font-semibold text-slate-400">{formatDateTime(summary?.lastChangedAt)}</p></td><td class="py-2.5 text-center">{#if isUnlocked}<form method="POST" action="?/disablePredictionUnlock" use:enhance><input type="hidden" name="userId" value={user.id} /><input type="hidden" name="tournamentId" value={data.selectedTournament?.id} /><button type="submit" class="rounded-lg bg-amber-100 px-3 py-1.5 text-xs font-bold text-amber-700 hover:bg-amber-200">Rebloquear</button>{#if unlock?.reason}<p class="mt-1 text-[10px] font-semibold text-slate-400" title={unlock.reason}>📝 {unlock.reason}</p>{/if}</form>{:else}<form method="POST" action="?/enablePredictionUnlock" use:enhance class="flex items-center justify-center gap-2"><input type="hidden" name="userId" value={user.id} /><input type="hidden" name="tournamentId" value={data.selectedTournament?.id} /><input name="reason" placeholder="Motivo" class="w-28 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 text-xs focus:border-emerald-400" /><button type="submit" class="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-700">Desbloquear</button></form>{/if}</td><td class="py-2.5 pr-4 text-right"><button onclick={() => startEdit(user)} class="rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-600 hover:bg-slate-200">✏️</button></td></tr>
